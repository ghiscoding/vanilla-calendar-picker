...
This parameter will only keep references of the date range edges which are the start and end dates in `settings.selected.dates` but nothing in between the range. It only works when <a href="/docs/reference/additionally/settings#selection-day">`settings.selection.day`</a> is set to `'multiple-ranged'`.

<Info>
  It's important to note that disabling dates inside the date range will have no effect and so you should use this parameter only when you are interested in the start and end date but nothing between.
</Info>
