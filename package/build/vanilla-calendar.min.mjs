...
const initSelectedMonthYear = (self) => {
  var _a;
  if (self.jumpToSelectedDate && ((_a = self.settings.selected.dates) == null ? void 0 : _a.length) && (self.settings.selected.month === void 0 && self.settings.selected.year === void 0)) {
...
const initRange = (self) => {
  ...
  if (self.settings.range.min === "today") {
    self.settings.range.min = (/* @__PURE__ */ new Date()).toISOString().substring(0, 10);
  }
  if (self.settings.range.max === "today") {
    self.settings.range.max = (/* @__PURE__ */ new Date()).toISOString().substring(0, 10);
  }
  ...
  self.rangeMin = isDisablePast ? getDateString(self.date.today) : self.settings.range.disableAllDays ? getDateString(self.date.today) : self.settings.range.min;
  self.rangeMax = self.settings.range.disableAllDays ? getDateString(self.date.today) : self.settings.range.max;
  self.rangeDisabled = self.settings.range.disabled && !self.settings.range.disableAllDays ? parseDates(self.settings.range.disabled) : self.settings.range.disableAllDays ? [self.rangeMin] : [];
  if (self.rangeDisabled.length > 1)
  ...
  if (((_a = self.rangeEnabled) == null ? void 0 : _a[0]) && ((_b = self.rangeDisabled) == null ? void 0 : _b[0]))
    self.rangeDisabled = self.rangeDisabled.filter((d) => !self.rangeEnabled.includes(d));
  if (self.rangeEnabled.length > 1)
...
const setDisabledDays = (self, date, dayWeekID) => {
  var _a, _b, _c, _d, _e;
  const isDisableWeekday = (_a = self.settings.range.disableWeekday) == null ? void 0 : _a.includes(dayWeekID);
  const isDisableAllDaysAndIsRangeEnabled = self.settings.range.disableAllDays && !!((_b = self.rangeEnabled) == null ? void 0 : _b[0]);
  if ((isDisableWeekday || isDisableAllDaysAndIsRangeEnabled) && !((_c = self.rangeEnabled) == null ? void 0 : _c.includes(date)) && !((_d = self.rangeDisabled) == null ? void 0 : _d.includes(date))) {
    self.rangeDisabled.push(date);
    (_e = self.rangeDisabled) == null ? void 0 : _e.sort((a, b) => +new Date(a) - +new Date(b));
  }
};
...
const DOMParser = (self, template) => template.replace(/[\n\t]/g, "").replace(/<#(?!\/?Multiple)(.*?)>/g, (_, p1) => {
  const component = getComponent(p1.replace(/[/\s\n\t]/g, ""));
  const htmlStr = component ? component(self) : "";
  return self.sanitizer(htmlStr);
}).replace(/[\n\t]/g, "");
...
const createYears = (self, target) => {
  ...
  for (let i = self.viewYear - 7; i < self.viewYear + 8; i++) {
    ...
    if (self.actions.getYears)
      self.actions.getYears(i, yearEl, self);
  }
};
...
const getLocaleWeekday = (self, i) => {
  const weekday = (/* @__PURE__ */ new Date(`1978-01-0${i + 1}T00:00:00.000Z`)).toLocaleString(self.settings.lang, { weekday: "short", timeZone: "UTC" });
  self.locale.weekday.push(capitalizeFirstLetter(weekday));
};
const getLocaleMonth = (self, i) => {
  const month = (/* @__PURE__ */ new Date(`1978-${i + 1 <= 9 ? `0${i + 1}` : i + 1}-01T00:00:00.000Z`)).toLocaleString(self.settings.lang, { month: "long", timeZone: "UTC" });
...
const createMonths = (self, target) => {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (self.actions.getMonths)
      self.actions.getMonths(i, monthEl, self);
  }
};
...
const setTime = (self, e, value, type) => {
  ...
  if (self.input && self.HTMLInputElement && self.actions.changeToInput)
    self.actions.changeToInput(e, self);
};
...
const handleDayRangedSelection = (self, formattedDate) => {
  ...
  if (formattedDate) {
    ...
    self.selectedDates = selectedDateExists && !self.settings.selection.cancelableDay ? [formattedDate, formattedDate] : selectedDateExists && self.settings.selection.cancelableDay ? [] : self.selectedDates.length > 1 ? [formattedDate] : [...self.selectedDates, formattedDate];
  ...
  const selectionHandlers = {
    ...
    reset: () => {
      ...
      self.selectedDates = self.selectedDates[0] !== self.selectedDates[self.selectedDates.length - 1] ? parseDates([`${startDate}:${endDate}`]) : [self.selectedDates[0], self.selectedDates[0]];
...
const handleDaySelection = (self, dayBtnEl, multiple) => {
  ...
  if (isSelected && !self.settings.selection.cancelableDay)
    return;
  let canToggle = true;
  if (self.settings.toggleSelected !== void 0) {
    canToggle = typeof self.settings.toggleSelected === "function" ? self.settings.toggleSelected(self) : self.settings.toggleSelected;
  }
...
const reset = (self, {
  ...
  self.settings.selected.dates = dates === "only-first" && ((_a = self.selectedDates) == null ? void 0 : _a[0]) ? [self.selectedDates[0]] : dates === true ? previousSelected.dates : self.selectedDates;
  setVariables(self);
  create(self);
  self.settings.selected = previousSelected;
  if (self.settings.selection.day === "multiple-ranged" && dates)
...
const handleInput = (self) => {
  ...
  const createCalendarToInput = () => {
    ...
    if (self.actions.initCalendar)
      self.actions.initCalendar(self);
...
const update = (self, {
  ...
  if (self.actions.updateCalendar)
    self.actions.updateCalendar(self);
};
const destroy = (self) => {
  var _a, _b, _c, _d;
  if (!self.isInit)
    throw new Error(messages.notInit);
  if (self.input) {
    (_b = (_a = self.HTMLElement) == null ? void 0 : _a.parentElement) == null ? void 0 : _b.removeChild(self.HTMLElement);
  ...
  if (self.actions.destroyCalendar)
    self.actions.destroyCalendar(self);
};
const show = (self) => {
  if (!self.currentType) {
